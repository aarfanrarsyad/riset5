    ...
    <div class="py-4">
        <div class="holder p-8 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-x-4">
            ...
            foreach ($galeri['foto'] as $foto) : ?>
                ...
                <div class="modal popUpFoto fixed overflow-auto top-0 bottom-0 right-0 left-0 z-40 bg-black bg-opacity-80 text-center font-paragraph hidden" id="popUp<?= $foto['id_foto']; ?>">
...
<script>
    //Button next dan prev pop up foto
    ...
    function suksesLaporFoto() {
        ...
        <div class="fixed top-0 bottom-0 right-0 left-0 z-50 flex justify-center items-center bg-black bg-opacity-40 font-paragraph" id='suksesLaporFoto'>
            <div class="hidden opacity-0 duration-700 transition-all p-3 rounded-lg flex items-center bg-greenAlert">
                <img src="/img/components/icon/check.png" class="h-5 mr-2 text-success" alt="icon check">
                <p class="sm:text-base text-sm font-heading font-bold text-success">Laporan Anda Berhasil Dikirim</p>
            </div>
        </div>
        `)
