...
<div class="xl:w-7/12 md:w-10/12 mt-8 flex items-center">
    <form method="POST" action="<?= route_to('reset-password') ?>" id="formResetPassword" class="rounded-3xl shadow-2xl xl:px-16 md:px-8 px-4 sm:mx-0 mx-4 w-full" data-aos="zoom-in">
        <?= csrf_field() ?>
        <h2 class="sm:text-2xl text-lg mb-3 font-bold text-center pt-8 text-black">ATUR ULANG KATA SANDI</h2>
        <div class="pl-3 rounded-full flex items-center 
            <?php if (session('error') == "Your password has been successfully changed. Please login with the new password.") : ?>
                is-invalid
            <?php else : ?>
                hidden
            <?php endif; ?>" style="background-color: #B1FF66;">
            <img src="/img/components/icon/check.png" class="h-5 mr-1" alt="icon check">
            <p class="sm:text-base text-sm" style="color: #54AC00;">Kata sandi Anda berhasil diubah. Silakan login dengan kata sandi baru.</p>
        </div>
        ...
        <div class="flex sm:h-10 h-8 mb-5">
            ...
            <input type="text" name="token" class="input pl-2 w-2/3 rounded-lg border-gray-400 outline-none text-gray-400 border-2" id="token" placeholder="abc123" value="<?= old('token', $token ?? '') ?>">
        </div>
        <div class="flex sm:h-10 h-8 mb-5">
            <label for="email" class="w-1/3 text-primary font-medium flex items-center sm:text-lg text-base">Alamat Email : </label>
            <input type="email" name="email" class="input pl-2 w-2/3 rounded-lg border-gray-400 outline-none text-gray-400 border-2" aria-describedby="emailHelp" id="email" placeholder="mail@example.com" value="<?= old('email') ?>">
        </div>
        <div class="flex sm:h-10 h-8 mb-5">
            <label for="password" class="w-1/3 text-primary font-medium flex items-center sm:text-lg text-base">Kata Sandi Baru : </label>
            <input type="password" name="password" class="input pl-2 w-2/3 rounded-lg border-gray-400 outline-none text-gray-400 border-2" id="password" placeholder="ketikkan kata sandi baru">
        </div>
        <div class="flex sm:h-10 h-8 mb-5">
            ...
            <input type="password" name="pass_confirm" class="input pl-2 w-2/3 rounded-lg border-gray-400 outline-none text-gray-400 border-2" id="pass_confirm" placeholder="konfirmasikan kata sandi baru">
        </div>
